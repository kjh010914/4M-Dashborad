...
def load_data():
    try:
        ...
        return pd.DataFrame(columns=["일시", "라인", "4M구분", "변동내용", "작업자", "품질내역", "유효성점검"])
    ...
    with st.form(key="input_form", clear_on_submit=True):
        ...
        if submit:
            if not change_detail or not worker_name:
                ...
                conn.update(worksheet="Sheet1", data=df_log)
                st.success(f"✅ [{selected_line}] 구글 스프레드시트 등록 완료!")
